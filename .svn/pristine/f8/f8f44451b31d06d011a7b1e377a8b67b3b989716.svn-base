...
public class AddAssetForPDP {
	...
	public void addImageToViewInPDP(AndroidDriver<AndroidElement> driver)
		...
		String comment = driver.findElement(pdp.pdpCommentView).getText();
		try {
	...
	public void addMultipleAssetInPDP(AndroidDriver<AndroidElement> driver)
			throws InterruptedException, MalformedURLException {
		wait = new WebDriverWait(driver, 10);
		cf.relaunchApp(driver);
		wait.until(ExpectedConditions.elementToBeClickable(ft.clickFavouriteTab));
		...
		for (int i = 0; i < 5; i++) {
			Thread.sleep(2000);
			driver.findElement(ft.editTextField).click();
			driver.findElement(ft.editTextField).sendKeys(comment + Integer.toString(i + 1));
			driver.hideKeyboard();
			Thread.sleep(2000);
			driver.findElement(pdp.saveNoteButton).click();
			wait.until(ExpectedConditions.invisibilityOfElementLocated(ft.sendTextProgressBar));
		...
		driver.findElement(pdp.pdpBackButton).click();
		try {
			wait.until(ExpectedConditions.elementToBeClickable(ft.showAllText));
			Thread.sleep(3000);
			driver.findElement(ft.showAllText).click();
		...
		Thread.sleep(3000);
		try {
			wait.until(ExpectedConditions.elementToBeClickable(ft.collapseAllButton));
			driver.findElement(ft.collapseAllButton).click();
